...
class BasicKotlinTests : KotlinTests() {
    ...
    @Test fun testTakeWhile() {
        Observable.fromIterable(listOf(1, 2, 3)).takeWhile { x -> x < 3 }.subscribe(::println)
        /*verify(a, times(1)).received(1)
        verify(a, times(1)).received(2)
        verify(a, times(0)).received(3)*/
    }
    ...
    @Test fun testLastOrDefault() {
        ...
        Observable.fromIterable(listOf("one", "two")).filter { it.length > 3 }.subscribe(::println)
    }
    ...
    @Test fun testAll() {
        Observable.fromIterable(listOf(1, 2, 3)).all { x -> x > 0 }.subscribe(::println)
//        verify(a, times(1)).received(true)
    ...
    @Test fun testZip() {
        ...
        val values = Observable.zip(o1, o2, o3, Function3<Int, Int, Int, List<Int>> { a, b, c -> listOf(a, b, c) }).subscribe(::println)
